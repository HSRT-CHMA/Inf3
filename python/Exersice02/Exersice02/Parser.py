...
class Parser(object):
    ...
    def parseExpression(self, line_input):
        ...
        no_brackets = ""
        # The version of line_input with no brackets in it
        
        print("This is the input for parseExpression : " + line_input)

        # Starts and Ends the line_input with brackets ?
        if line_input.endswith(")") and line_input.startswith("("):
            print("Its a 'Factor-Expression' !")
            no_brackets = line_input[1:len(line_input)-1]
            print("No brackets input : " + no_brackets)
            # Does it contains no + or multiple + ?
            if '+' in no_brackets :
                print("Multiple Terms")
                all_term = no_brackets.split('+') 
                ...
                print("Single Term")
                if self.parseTerm(line_input) != None :
                    numeric_result = self.parseTerm(line_input)
            ...
            print("Its a normal Equation-Expression")
